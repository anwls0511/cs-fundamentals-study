...
IP와 port에 대해서는 이렇게 설명할 수 있다.

> IP는 네트워크에서 컴퓨터를 찾기 위한 주소이고, port는 그 컴퓨터 안에서 특정 서버 프로그램을 찾기 위한 번호입니다. 예를 들어 `127.0.0.1:8080`은 내 컴퓨터의 8080번 port에서 기다리는 서버를 의미합니다.

socket에 대해서는 이렇게 설명할 수 있다.

> socket은 애플리케이션이 네트워크 통신을 하기 위해 사용하는 통신 endpoint입니다. 서버는 `ServerSocket`으로 특정 port를 열고 기다리고, 클라이언트는 서버 IP와 port로 접속해서 데이터를 주고받습니다.

TCP 연결에 대해서는 이렇게 설명할 수 있다.

> TCP 서버는 port를 열고 대기하고, 클라이언트가 해당 IP와 port로 접속하면 연결이 만들어집니다. 연결 이후에는 양쪽 socket의 input stream과 output stream을 통해 바이트 데이터를 주고받습니다.
