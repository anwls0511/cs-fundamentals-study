...
Blocking I/O에서는 read 같은 호출이 데이터가 올 때까지 현재 thread를 멈춰 기다리게 한다. 연결마다 thread를 하나씩 쓰는 구조는 단순하지만, 연결이 많아지면 thread 수와 context switching 비용이 늘어날 수 있다. Netty는 이런 문제를 줄이기 위해 non-blocking I/O와 이벤트 루프 기반 구조를 사용한다.
